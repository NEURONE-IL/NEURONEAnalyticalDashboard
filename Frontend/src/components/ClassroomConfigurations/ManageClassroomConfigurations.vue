<template>
    ...
    <v-row no-gutters>
      <v-col cols="12" class="text-center">
				<h2>Administrar configuraciones de aula</h2>
				<br>
				<v-divider></v-divider>	
				<br>				
				<v-form
					ref="updatedConfigurationForm"
					v-model="updatedValidConfiguration"
				>
					<v-row no-gutters>
						<v-col cols="12" class="text-center">
							<v-card>
				...
				<v-btn
					class="mb-4 ms-4"
					color="primary"
					:disabled="!updatedValidConfiguration || this.participants === 0"
					@click="setClassroom([])"
				>
...
export default {
	...
	data (){
		return {
			...
			positions: [],
			resolution: '',
			updatedValidConfiguration: true,
	...
	mounted(){
		...
		this.maxRadius = 15;
		this.separation = 50;
		this.classroomConfigurations = this.$store.getters.getClassroomConfigurations;
	},
...
/*			var selectedNodes = [];
		...
		print(){
			...
			for(var i = 0; i < this.participants; i++){
				positions.push(
					{
						x: this.chart.series.values[0].dataItems.values[i].node.x,
						y: this.chart.series.values[0].dataItems.values[i].node.y,
					}
				);
			}
		...
		async saveConfiguration(){
			this.resolution = this.$refs.chartdiv.clientWidth;
			for(var i = 0; i < this.participants; i++){
				this.positions.push(
					[
